...
Stack - коллекция основанная на списке, т.к. работает по принципу LIFO, следовательно для работы с последними эллементами коллекции удобнее всего использовать именно
ее всего использовать именно списки. 

Методы (push - метод добавления элемента в конец коллекции pop - метод удаления элемента с конца коллекции peek - метод, возвращающий вершинный элемент стека) 

Queue - коллекция основанная на deque (коллекция на основе кусочных данных, в виде связанных коллекциями определенной длины), основной принцип работы FIFO - работаем с первым эллементом. 

Методы (enqueue - метод добавление элемента в начало коллекции dequeue - метод удаление получения и удаления элемента с начала коллекции peek - метод, возвращающий первого в очереди )

SingleList - коллекция из связанных элементов - Node(value, linknextel). 

Методы (append - метод, добавляющий элемент в конец односвязного списка prepand - метод, добавляющий элемент в начало insert - метод, добавляющий элемент на произвольную позицию, изменяя указатель предыдущего элемента )
...
![alt text](images/lab07/test_text.png)
